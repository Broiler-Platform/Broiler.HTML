...
internal static class RendererStyleQueries
{
    internal sealed record FontFace(string Family, string Source, string FeatureSettings);
    ...
    internal static IReadOnlyList<FontFace> GetFontFaces(CssStyleSheet styleSheet) =>
        Cache.GetValue(styleSheet, ParseMetadata).FontFaces;

    internal static IReadOnlyDictionary<string, Dictionary<string, Dictionary<string, int[]>>> GetFontFeatureValues(
        CssStyleSheet styleSheet) =>
        Cache.GetValue(styleSheet, ParseMetadata).FontFeatureValues;

    internal static bool HasGeneratedPseudoElementRules(CssStyleSheet styleSheet, bool before)
    {
