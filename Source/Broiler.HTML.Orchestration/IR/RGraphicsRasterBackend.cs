...
internal sealed class RGraphicsRasterBackend : IRasterBackend
{
    ...
    private static void RenderDrawText(RGraphics g, DrawTextItem item)
    {
        ...
        if (item.FontHandle is RFont font)
        {
            ...
            else
            {
                ...
                // draws stay upright.
                VerticalGlyphContext.RotationDeg = item.GlyphRotationDeg;
                try
                {
                ...
                {
                    VerticalGlyphContext.RotationDeg = 0f;
                }
            }
        }
    }
    ...
    private static void RenderDrawTiledGradient(RGraphics g, DrawTiledGradientItem item)
    {
        ...
        using var tileImage = item.IsConic
            ? g.CreateConicGradientTile(tileW, tileH, colors, positions, item.CenterX, item.CenterY, item.FromAngle)
            : item.IsRadial
