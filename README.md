...
- the non-timeout visual mismatches have clustered around a small `css/css-backgrounds` subset (`background-attachment-*`, `background-334.html`, and a few `background-clip/*` cases such as `clip-border-area.html`, the `clip-border-area-*` variants, `clip-border-box.html`, `clip-border-shape-table-part-background.html`, `clip-rounded-corner.html`, `clip-text-background-table-cell.html`, `clip-text-constrain-geometry.html`, `clip-padding-box.html`, and the `clip-content-box*.html`/`clip-padding-box_with*.html` tests), so the curated CI slice currently excludes those known unstable cases while renderer gaps are investigated separately
