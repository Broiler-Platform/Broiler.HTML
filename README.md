...
- the non-timeout visual mismatches have clustered around a small `css/css-backgrounds` subset (`background-attachment-*`, `background-334.html`, and `background-clip/clip-border-area-border-image.html`), so the curated CI slice currently excludes those known unstable cases while renderer gaps are investigated separately
